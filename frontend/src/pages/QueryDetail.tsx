...
import { getSlowQueryDetail, analyzeSpecificQuery, analyzeQueryWithAI } from '../services/api';
...
const QueryDetail: React.FC = () => {
  ...
  const handleAnalyzeAI = async (force?: boolean) => {
    if (!id) return;
    try {
      setAiAnalyzing(true);
      const result = await analyzeQueryWithAI(id, { force });
      setQuery(prev =>
        prev
          ? {
              ...prev,
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
              ...
              <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-4 gap-3">
                <div>
                  <h2 className="text-xl font-semibold flex items-center space-x-3">
                    ...
                    <span
                      className={`badge ${getAnalysisMethodBadge(query.analysis.analysis_method)}`}
                    >
                      {getAnalysisMethodLabel(query.analysis.analysis_method)}
                    </span>
                  </h2>
                  <p className="text-sm text-gray-500 mt-1">
                    Generated by the heuristic analyzer (version {query.analysis.analyzer_version || 'N/A'})
                  </p>
                </div>
                <span
              ...
              <div className="space-y-4">
                ...
                <div className="pt-4 border-t border-gray-200">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-gray-600">
                      Method:{' '}
                      <strong>{getAnalysisMethodLabel(query.analysis.analysis_method)}</strong>
                    </span>
                  </div>
                </div>
              </div>
        ...
        <div className="mt-6">
          <AIAnalysisSection
            analysis={query.ai_analysis ?? null}
            loading={aiAnalyzing}
